...
for transaction in daily_transactions_split:
  ...
  transactions_clean.append(newline)
...
for thread in thread_sold:
  if "&" in thread:
    thread = thread.split("&")
    thread_sold_split.append(thread[0])
    thread_sold_split.append(thread[1])
    if len(thread) >= 3:
      thread_sold_split.append(thread[2])
    if len(thread) == 4:
      thread_sold_split.append(thread[3])
    continue
